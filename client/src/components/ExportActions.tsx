...
interface ExportActionsProps { title: string; rows: Record<string, Cell>[]; columns: ExportColumn<Record<string, Cell>>[]; filters?: Record<string, string>; logoUrl?: string; }

export default function ExportActions({ title, rows, columns, filters = {}, logoUrl }: ExportActionsProps) {
  ...
  const startExport = async () => {
    ...
    if (format === "csv") exportRowsToCsv(exportTitle, rows, activeColumns);
    else await exportRowsToPdf(exportTitle, rows, activeColumns, options);
    setLoading(false);
    setOpen(false);
  };
