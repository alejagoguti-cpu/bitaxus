...
export function useReconciliationSupabase(tenantId?: string) {
  return useQuery<PublicReconciliationRow[]>({ queryKey: ["public-reconciliation", tenantId ?? "anonymous"], queryFn: async () => { const { data, error } = await supabase.from("reconciliation").select("*").order("created_at", { ascending: false }).limit(200); if (error) throw error; return (data ?? []) as PublicReconciliationRow[]; }, // RLS filters rows by auth.uid(); tenantId is presentation metadata only.
    enabled: true, retry: false, staleTime: 30000 });
}
