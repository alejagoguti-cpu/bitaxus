...
import { ArrowDownLeft, ArrowLeftRight, ArrowUpRight, CheckCircle2, ChevronRight, Download, Globe2, Info, LoaderCircle, Search, Send, Star, X } from "lucide-react";
...
type CurrencyAction = GlobalOperationType | "Configuración";
...
export default function Global({ onNavigate }: { onNavigate: (section: string) => void }) {
  ...
  const [currencyAction, setCurrencyAction] = useState<CurrencyAction | null>(null);
  ...
  const openCurrencyPicker = (action: CurrencyAction = "Configuración") => {
  ...
  const chooseCurrency = (currency: GlobalCurrency) => {
    ...
    window.setTimeout(() => {
      ...
      setCurrencyQuery("");
      setSelectedCurrencyCode(null);
      if (action !== "Configuración") open(action, currency.code);
    }, 180);
  };
        ...
        <main>
          ...
          <div className="balance-grid">
            ...
            <article className="balance-card currency-config-card">
              <div className="balance-icon"><Globe2 size={23} /></div><span>Moneda de la transacción</span><strong>{quickCurrency.code}</strong><small>{quickCurrency.name}</small>
              <button className="configure-currency" type="button" onClick={() => openCurrencyPicker()}>Configurar moneda <ChevronRight size={15} /></button>
              <div className="balance-actions" aria-label={`Operaciones en ${quickCurrency.code}`}><button onClick={() => open("Recepción", quickCurrency.code)}><ArrowDownLeft size={15} /> Recibir</button><button onClick={() => open("Conversión", quickCurrency.code)}><ArrowLeftRight size={15} /> Convertir</button><button onClick={() => open("Dispersión", quickCurrency.code)}><Send size={15} /> Dispersar</button></div>
            </article>
