        ...
        <main>
          ...
          <div className="balance-grid">
            ...
            <article className="balance-card currency-config-card">
              <div className="currency-pair-heading"><span>Par de monedas</span><strong>{sourceCurrency.code} <ArrowRight size={17} /> {targetCurrency.code}</strong><button type="button" className="swap-currency-pair" onClick={swapCurrencyPair} aria-label={`Intercambiar ${sourceCurrency.code} y ${targetCurrency.code}`} title="Intercambiar origen y destino"><ArrowLeftRight size={15} /></button></div>
              ...
              <div className="global-operation-choice" role="group" aria-label="Selecciona la operación"><button type="button" className={operationIntent === "Recepción" ? "selected" : ""} onClick={() => setOperationIntent("Recepción")}><ArrowDownLeft size={15} /> Recibir</button><button type="button" className={operationIntent === "Conversión" ? "selected" : ""} onClick={() => setOperationIntent("Conversión")}><ArrowLeftRight size={15} /> Convertir</button><button type="button" className={operationIntent === "Dispersión" ? "selected" : ""} onClick={() => setOperationIntent("Dispersión")}><Send size={15} /> Dispersar</button></div>
              <label className="global-operation-amount"><span>{selectedOperationLabel}</span><div><b aria-hidden="true">{selectedOperationCurrency.symbol}</b><input type="number" inputMode="decimal" min="0.01" step="any" value={operationAmount} onChange={event => setOperationAmount(event.target.value)} placeholder="0,00" aria-label={`${selectedOperationLabel} en ${selectedOperationCurrency.code}`} /><small>{selectedOperationCurrency.code}</small></div></label>
              {operationIntent === "Conversión" && <section className={`global-converted-preview${convertedAmount !== null ? " is-ready" : ""}`} aria-live="polite"><span>Resultado estimado</span><strong key={convertedAmount ?? "empty"}>{convertedAmount !== null ? money(convertedAmount, targetCurrency.code) : `Ingresa un importe en ${sourceCurrency.code}`}</strong><small>{convertedAmount !== null ? `Con la tasa de referencia hacia ${targetCurrency.code}` : "Se actualizará automáticamente al escribir"}</small></section>}
              <button type="button" className="global-operation-continue" onClick={startSelectedOperation}>Continuar con {operationIntent.toLocaleLowerCase("es-CO")} <ArrowRight size={15} /></button>
            </article>
