...
export default function Global({ onNavigate }: { onNavigate: (section: string) => void }) {
  ...
  const open = (type: GlobalOperationType, sourceCode = form.source_currency, targetCode = form.target_currency) => {
    const numericAmount = Number(operationAmount);
    ...
    update("source_amount", operationAmount);
    update("target_amount", type === "Conversión" && quoteIsCurrent ? String(numericAmount * referenceQuote.rate) : "");
  ...
  return (
    <section className="global-page">
      ...
      <div className="global-layout">
        <main>
          <section className="global-operation-shell">
            <header className="global-operation-header"><div><span className="global-kicker">Operación internacional</span><h3>Define el par y el importe</h3><p>Configura el movimiento en un solo paso. Podrás revisar todo antes de confirmar.</p></div><div className="global-live-chip"><i /> Tasa de referencia</div></header>
            <div className="global-operation-choice" role="group" aria-label="Selecciona la operación"><button type="button" className={operationIntent === "Recepción" ? "selected" : ""} onClick={() => setOperationIntent("Recepción")}><ArrowDownLeft size={15} /><span>Recibir</span></button><button type="button" className={operationIntent === "Conversión" ? "selected" : ""} onClick={() => setOperationIntent("Conversión")}><ArrowLeftRight size={15} /><span>Convertir</span></button><button type="button" className={operationIntent === "Dispersión" ? "selected" : ""} onClick={() => setOperationIntent("Dispersión")}><Send size={15} /><span>Dispersar</span></button></div>
            <div className="global-operation-grid">
              <section className="global-pair-stage" aria-label="Monedas de la operación"><div className="global-stage-label"><span>Tu par de monedas</span><button type="button" className="swap-currency-pair" onClick={swapCurrencyPair} aria-label={`Intercambiar ${sourceCurrency.code} y ${targetCurrency.code}`} title="Intercambiar origen y destino"><ArrowLeftRight size={15} /></button></div><div className="currency-pair-controls"><button className="configure-currency" type="button" onClick={() => openCurrencyPicker("Origen")}><span aria-hidden="true" className="currency-flag">{sourceCurrency.flag}</span><span><small>Origen</small><b>{sourceCurrency.code}</b><em>{sourceCurrency.name}</em></span><ChevronRight size={15} /></button><ArrowRight className="pair-route-arrow" size={18} /><button className="configure-currency" type="button" onClick={() => openCurrencyPicker("Destino")}><span aria-hidden="true" className="currency-flag">{targetCurrency.flag}</span><span><small>Destino</small><b>{targetCurrency.code}</b><em>{targetCurrency.name}</em></span><ChevronRight size={15} /></button></div><div className="global-balance-strip"><div><span>Disponible en {sourceCurrency.code}</span><b>{money(balances[sourceCurrency.code] ?? 0, sourceCurrency.code)}</b></div><div><span>Disponible en {targetCurrency.code}</span><b>{money(balances[targetCurrency.code] ?? 0, targetCurrency.code)}</b></div></div></section>
              <section className="global-amount-stage"><label className="global-operation-amount"><span>{selectedOperationLabel}</span><div><b aria-hidden="true">{selectedOperationCurrency.symbol}</b><input type="number" inputMode="decimal" min="0.01" step="any" value={operationAmount} onChange={event => setOperationAmount(event.target.value)} placeholder="0,00" aria-label={`${selectedOperationLabel} en ${selectedOperationCurrency.code}`} /><small>{selectedOperationCurrency.code}</small></div></label>{operationIntent === "Conversión" && <section className={`global-converted-preview${convertedAmount !== null ? " is-ready" : ""}`} aria-live="polite"><span>Recibirás aproximadamente</span><strong key={convertedAmount ?? "empty"}>{convertedAmount !== null ? money(convertedAmount, targetCurrency.code) : `— ${targetCurrency.code}`}</strong><small>{convertedAmount !== null ? `Resultado estimado al tipo de cambio actual` : "El resultado se calcula al ingresar un importe"}</small></section>}<button type="button" className="global-operation-continue" onClick={startSelectedOperation}>Revisar {operationIntent.toLocaleLowerCase("es-CO")} <ArrowRight size={15} /></button></section>
              <section className={`reference-quote ${quoteStatus}`} aria-live="polite"><span>Cotización de referencia</span>{quoteStatus === "loading" && <b><LoaderCircle size={15} className="spin" /> Consultando tasa…</b>}{quoteStatus === "ready" && referenceQuote && <><b>1 {sourceCurrency.code} = {rateLabel(referenceQuote.rate)} {targetCurrency.code}</b><small>Publicada el {dateLabel(referenceQuote.date)} · Frankfurter</small></>}{quoteStatus === "error" && <b>No se pudo consultar la tasa para este par.</b>}<button type="button" onClick={() => setQuoteRefreshKey(value => value + 1)} aria-label={quoteStatus === "loading" ? "Actualizando cotización de referencia" : "Actualizar cotización de referencia"} aria-busy={quoteStatus === "loading"} disabled={quoteStatus === "loading"}>{quoteStatus === "loading" ? <><LoaderCircle size={13} className="spin" /> Actualizando…</> : <><RefreshCw size={13} /> Actualizar</>}</button></section>
            </div>
          </section>
      ...
      {modal && <div className="modal-backdrop" onClick={() => !createOperation.isPending && setModal(null)}><div className="action-modal global-modal global-confirmation-modal" onClick={event => event.stopPropagation()}><button className="modal-close" onClick={() => !createOperation.isPending && setModal(null)} aria-label="Cerrar"><X size={17} /></button><div className="modal-icon"><Globe2 size={17} /></div><h2>Revisa tu operación</h2><p>Confirma que el valor y las monedas coincidan antes de finalizar.</p><div className="global-operation-summary"><span className="global-summary-kicker">{modal === "Conversión" ? "Conversión preparada" : modal === "Recepción" ? "Recepción preparada" : "Dispersión preparada"}</span><div className="global-summary-route"><div className="global-summary-currency"><i aria-hidden="true">{confirmationSourceCurrency.flag}</i><span><small>{modal === "Recepción" ? "Vas a recibir" : "Monto de origen"}</small><b>{money(Number(form.source_amount), form.source_currency)}</b></span></div>{modal === "Conversión" && <><ArrowRight size={18} className="global-summary-arrow" /><div className="global-summary-currency target"><i aria-hidden="true">{confirmationTargetCurrency.flag}</i><span><small>Recibirás estimado</small><b>{money(Number(form.target_amount), form.target_currency)}</b></span></div></>}</div>{modal === "Conversión" && <div className="global-summary-rate"><span>Tasa de referencia</span><b>1 {form.source_currency} = {rateLabel(Number(form.exchange_rate))} {form.target_currency}</b></div>}<small className="global-summary-context">{modal === "Recepción" ? `Moneda destino · ${form.source_currency}` : modal === "Dispersión" ? `Moneda origen · ${form.source_currency}` : `${form.source_currency} → ${form.target_currency}`}</small></div><div className="modal-actions"><button className="secondary-action" onClick={() => setModal(null)}>Volver</button><button className="primary-action" onClick={submit} disabled={createOperation.isPending}>{createOperation.isPending ? <><LoaderCircle size={14} className="spin" /> Guardando...</> : "Confirmar operación"}</button></div></div></div>}
    </section>
  );
}
