...
export default function Global({ onNavigate }: { onNavigate: (section: string) => void }) {
  ...
  return (
    <section className="global-page">
      ...
      {modal && <div className="modal-backdrop" onClick={() => !createOperation.isPending && setModal(null)}><div className="action-modal global-modal global-confirmation-modal" onClick={event => event.stopPropagation()}><button className="modal-close" onClick={() => !createOperation.isPending && setModal(null)} aria-label="Cerrar"><X size={17} /></button><div className="modal-icon"><Globe2 size={17} /></div><h2>Confirmar {modal.toLocaleLowerCase("es-CO")}</h2><p>El importe y las monedas ya fueron definidos en la tarjeta Global.</p><div className="global-operation-summary"><span>{modal === "Conversión" ? "Vas a convertir" : modal === "Recepción" ? "Vas a recibir" : "Vas a dispersar"}</span><strong>{money(Number(form.source_amount), form.source_currency)}{modal === "Conversión" && <><ArrowRight size={16} /> {money(Number(form.target_amount), form.target_currency)}</>}</strong>{modal === "Conversión" && <small>Tasa de referencia: 1 {form.source_currency} = {rateLabel(Number(form.exchange_rate))} {form.target_currency}</small>}<small>{modal === "Recepción" ? `Moneda destino · ${form.source_currency}` : modal === "Dispersión" ? `Moneda origen · ${form.source_currency}` : `${form.source_currency} → ${form.target_currency}`}</small></div><div className="modal-actions"><button className="secondary-action" onClick={() => setModal(null)}>Volver</button><button className="primary-action" onClick={submit} disabled={createOperation.isPending}>{createOperation.isPending ? <><LoaderCircle size={14} className="spin" /> Guardando...</> : "Confirmar operación"}</button></div></div></div>}
    </section>
  );
}
