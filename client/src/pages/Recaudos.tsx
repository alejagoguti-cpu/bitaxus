...
type ReceiptRow = { id: string; payer: string; concept: string; amount: string; date: string; status: string; currency: string; createdAt: string };
...
export default function Recaudos({ onNavigate }: { onNavigate: (section: string) => void }) {
  ...
  const payersQuery = useQuery({ queryKey: ["public-payers"], queryFn: async () => { const { data, error } = await supabase.from("payers").select("id,name,id_type,identification_number,payer_type,email,phone,created_at").order("created_at", { ascending: false }).limit(100); if (error) throw error; return data ?? []; }, staleTime: 30000, retry: false });
  useEffect(() => { if (receiptsQuery.data) setReceipts(receiptsQuery.data.map(row => ({ id: row.id, payer: row.payer_name || "", concept: row.concept || "", amount: `$ ${Number(row.amount || 0).toLocaleString("es-CO")}`, date: new Date(row.receipt_date || row.created_at).toLocaleDateString("es-CO", { day: "2-digit", month: "short", year: "numeric" }), status: row.status || "", currency: String(row.currency || "COP").toUpperCase(), createdAt: row.created_at }))); }, [receiptsQuery.data]);
  useEffect(() => { if (payersQuery.data) setPayers(payersQuery.data.map(row => ({ name: row.name || "", idType: row.id_type || "", id: row.identification_number || "", email: row.email || undefined, phone: row.phone || undefined, type: row.payer_type === "Persona natural" ? "Persona natural" : "Persona jurídica" }))); }, [payersQuery.data]);
  ...
  const savePayer = async () => {
    ...
    const { data, error } = await supabase.from("payers").insert({ name: newPayerName.trim(), id_type: newPayerIdType === "Cédula de ciudadanía" ? "CC" : newPayerIdType, identification_number: newPayerId.trim(), payer_type: newPayerType, email: newPayerEmail.trim() || null, phone: newPayerPhone.trim() || null, created_by_open_id: authData.user.id, created_by_name: authData.user.user_metadata?.name || authData.user.email || null }).select("id,name,id_type,identification_number,payer_type,email,phone").single();
    ...
    const created: Payer = { name: data.name || "", idType: data.id_type || "", id: data.identification_number || "", type: data.payer_type === "Persona natural" ? "Persona natural" : "Persona jurídica", email: data.email || undefined, phone: data.phone || undefined };
  ...
  return <section className="receipts-page">
    ...
    {selectedReceipt && <OperationDetailModal kind="Recaudo" tone="receipt" title={selectedReceipt.concept || "Recaudo"} subtitle={selectedReceipt.payer || "Pagador sin nombre"} metricLabel="Valor de la operación" metricValue={selectedReceipt.amount} fields={[{ label: "Estado", value: <span className={`receipt-status ${selectedReceipt.status.toLowerCase()}`}>{selectedReceipt.status}</span> }, { label: "Fecha de registro", value: selectedReceipt.date }, { label: "Moneda", value: selectedReceipt.currency }, { label: "Referencia", value: selectedReceipt.id }]} onClose={() => setSelectedReceipt(null)} />}
    {!formOpen && <button className="floating-add primary-action" onClick={() => onNavigate("Programar recaudo")}><Plus size={16}/> Programar recaudo</button>}
